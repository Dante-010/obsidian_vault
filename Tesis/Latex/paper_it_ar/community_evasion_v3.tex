...
Critic head        & Graph-level A2C value head & Target-conditioned + attention pooling \\
Scalability        & Graph-wide computation & Policy-Guided Subgraphing \\
\bottomrule
\end{tabular}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.45\linewidth}
    ...
    \par\smallskip
    {\small\textit{Original.} Target node conditioning is applied via
      an external environment mask rather than inside the network.}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.45\linewidth}
    \centering
    ...
    \par\smallskip
    {\small\textit{Ours (v1).} The target embedding $h_t$ is injected
      explicitly into both Actor and Critic heads.}
  \end{minipage}
...
our model achieves near-perfect success rates, while at lower budget levels
the gap between the two architectures is more pronounced. Importantly, the proposed architecture
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.92\linewidth]{figures/tradeoff_plot}
...
\textit{(This section presents 3D surface plots showing how each evaluation
metric varies jointly with the hiding threshold $\tau$ and the rewiring budget
$\beta$, for the GATv2 v1 agent on the \texttt{words} dataset (Louvain
community detection). One surface is shown per metric: F1 score, NMI, and
Success Rate. Results for other datasets and algorithms follow the same
qualitative trends.)}

To better understand the joint sensitivity of the proposed architecture to
$\tau$ and $\beta$, we swept over $\tau \in \{0.3, 0.5, 0.8\}$ and
$\beta \in \{0.5, 1.0, 2.0\}$. In general, larger $\beta$ (more budget) and
larger $\tau$ (softer hiding criterion) make the task easier, and all three
surfaces reflect this trend. NMI tends to decrease as the budget grows, since
more rewiring steps introduce more structural perturbation. The most
informative regime is around intermediate values ($\tau \approx 0.5$,
$\beta \approx 1.0$), which best captures the tradeoff between hiding
effectiveness and graph preservation.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{sweep_words_louvain_f1_v1}
  \caption{F1 score surface over $(\tau, \beta)$ — \texttt{words} dataset,
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{sweep_words_louvain_nmi_v1}
  \caption{NMI surface over $(\tau, \beta)$ — \texttt{words} dataset,
    Louvain community detection. Higher NMI indicates lower structural
    distortion.}
  \label{fig:sweep-nmi}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\linewidth]{sweep_words_louvain_success_rate_v1}
  \caption{Success Rate surface over $(\tau, \beta)$ — \texttt{words} dataset,
    Louvain community detection.}
  \label{fig:sweep-sr}
\end{figure}
